...
class InterbreedingVisualizer:
    ...
    def plot_all(self):
        ...
        female_history = [item['female_count']
                          for item in self.population.history]
        years = range(1, self.population.elapsed_years + 1)

        ax1.plot(
            years, male_history[1:], label='Male Population',
            linestyle='--', color='blue')
        ax1.plot(
            years, female_history[1:], label='Female Population',
            linestyle='-', color='red')
        ...
        age_labels = [f'{age_bins[i]}-{age_bins[i + 1] - 1}'
        ...
        ax2.barh(
            y_positions, male_counts, color='blue', alpha=0.7, label='Men')
        ax2.barh(
            y_positions, female_counts, color='pink', alpha=0.7, label='Women')
        ...
        ax2.xaxis.set_major_formatter(
            FuncFormatter(lambda x, _: f'{abs(int(x))}'))
        ...
        green_std_devs = [item['green_std_dev']
                          for item in self.population.history]
        brown_std_devs = [item['brown_std_dev']
                          for item in self.population.history]

        ax3.plot(
            years, green_means[1:], label='Average Green Proportion',
            linestyle='-', color='green')
        ax3.plot(years, brown_means[1:], label='Average Brown Proportion',
                 linestyle='-', color='brown')

        ax3.plot(
            years, green_std_devs[1:], label='Green Std Dev',
            linestyle='--', color='darkgreen')
        ax3.plot(
            years, brown_std_devs[1:], label='Brown Std Dev',
        ...
        sort_order = np.argsort(genetic_data[0])[::-1]
        genetic_data_sorted = genetic_data[:, sort_order]
        indices_sorted = np.arange(len(self.population.members))

        ax5.bar(
            indices_sorted, genetic_data_sorted[0],
            label='Green', color='green', alpha=0.7)
        ax5.bar(
            indices_sorted, genetic_data_sorted[1],
            bottom=genetic_data_sorted[0],
            label='Brown', color='brown', alpha=0.7)
    ...
    def plot_population_growth(self):
        ...
        years = range(1, self.population.elapsed_years + 1)
        plt.plot(
            years, male_history[1:],
            label='Male Population', linestyle='--', color='blue')
        plt.plot(
            years, female_history[1:],
            label='Female Population', linestyle='-', color='red')
